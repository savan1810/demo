    ...
    <div className="bg-gradient-to-tl from-blue-50 overflow-x-hidden via-blue-50 to-blue-50 blur-3xl  w-[98vw] h-[50rem] rounded-fulls origin-top-left -rotate-12 -translate-x-[15rem] "></div>
        ...
        <div className='grid lg:grid-cols-2 grid-cols-1 h-full w-full'>
          <div className='  w-full h-full hidden lg:flex lg:flex-col ml-20 justify-center '>
            ...
            <p className='text-4xl mt-4 font-semibold text-black'>Create account</p>
          </div>
          <div className=' lg:w-3/5 w-full border  bg-white shadow-xl  rounded-2xl mx-auto flex justify-center items-center  sm:my-10 mt-8 mb-32 '>
            <div className=' z-10 px-4  w-full mx-auto flex flex-col items-center justify-center sm:gap-10 gap-20  '>
              <div className='text-2xl   mb-4 text-center  tracking-wide'>
                ...
                <p className="mt-4 text-xl">Let&lsquo;s create account</p>
              </div>
              <form className=" mx-auto w-full ">
                 <div className='mb-2'>
                    <TextField fullWidth label="Email" onChange={(e) => setEmail(e.target.value)} color="" id="fullWidth" />
                 </div> 
                 <div className='mt-2'>
                    <TextField fullWidth label="Password" onChange={(e) => setPassword(e.target.value)} id="fullWidth" className=''/>
                 </div>
                 <div className='mt-2'>
                    <TextField fullWidth label="Confirm Password"  onChange={(e) => setPasswordAgain(e.target.value)} id="fullWidth" className=''/>
                 </div>
                
                <div className='w-full py-4 text-center'>
                                <button  disabled={(!email || !password || !passwordAgain) || (password !== passwordAgain)}  onClick={signup} className='w-full bg-gradient-to-r from-[#00A9FF] to-[#A0E9FF]  my-2 text-xl p-4  outline-0 text-white rounded-md' >Sign up</button>
                            </div>
